...
///Method For Stanford code exist starts
     ...
     *    or MissingViewException in debug mode.
     */
	public function add_stock($stock_data,$access_token,$access_array)
	{
	     try {
                ...
                $insert_res = $this->db->insert($this->stock, $stock_data);
                ...
                $user_id = $this->getUserId($where_array);
                //echo $user_id;exit;
                $user_session_data['useri_id'] = $user_id;

                $user_session_info = $this->AddUserSession($user_session_data);
                //print_r($user_session_info);exit;

                //$this->db->where('id',$user_id);
                //$this->db->set('user_session_id', $user_session_info['user_session_id']);
		        ...
		        if ($this->db->trans_status() === FALSE)  {
		            ...
		            //$user_session_id;exit;
		            return array('access_token' => $user_session_info['access_token']);
	...
	/**
	 * Get user id of an email
     * @param  array $where_array consists of email of the user
     * @return id of the given email
     * @throws NotFoundException When the view file could not be found
     *    or MissingViewException in debug mode.
	**/
	public function getUserId($where_array) {
		try
		{ 
		 	$this->db->select('id');
			$this->db->from($this->users);
	...
	public function createRefferal($user_data){
	...
	public function AddUserSession($user_session_data)
	{
	      try {
                ...
                $user_session_data['row_status'] = 1;
                //$user_session_data['login_time'] = date("Y-m-d H:i:s");
                $user_session_data['created_at'] = date("Y-m-d H:i:s");
                $user_session_data['access_token'] = md5(md5($user_session_data['user_id'].strtotime(date("Y-m-d H:i:s"))));
		        ...
		        if ($this->db->trans_status() === FALSE) {
		            ...
		            $user_session_data['user_session_id'] = $this->getUserSessionId($user_session_data);//exit;
		            return $user_session_data;//array('user_session_id'=>$user_session_id);
		        }
            } catch (Exception $e) {
			    log_message('error', $this->db->_error_message());
                $this->db->trans_rollback();
...
/**
	...
	public function getUserSessionId($user_session_data){
		try {
			...
			$this->db->where($user_session_data); 
